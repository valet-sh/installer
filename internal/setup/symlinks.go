...
func CreateSymlinks(vshUser string) error {
    localBinPath := "/usr/local/bin"
    if _, err := os.Stat(localBinPath); os.IsNotExist(err) {
        if err := utils.RunCommand("sudo", []string{"mkdir", "-p", localBinPath}); err != nil {
            return fmt.Errorf("failed to create local bin directory: %w", err)
        }
        if err := utils.RunCommand("sudo", []string{"chown", vshUser, localBinPath}); err != nil {
    ...
    if err := utils.RunCommand("sudo", []string{"ln", "-sf", vshBinPath, "/usr/local/bin/valet.sh"}); err != nil {
